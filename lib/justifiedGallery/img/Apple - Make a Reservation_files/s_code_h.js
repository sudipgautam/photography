...
// 2008-04-03: Genesis DFA integration plugin, doPlugins function call, s.variableProvider
// 2008-08-15: Upgraded to H.17 base code,updated current plugins,added detectRIA, and removed legacy flash detection function, added enhanced download, internal search event
// 2008-12-19: Removed all document.title references, handled cases where pageName not set
// 2009-02-05: Added CID check to file instead of external JS, cleaned up comments
// 2009-03-01: Upgraded to H.19.4 base code
// 2009-04-21: Added permanent logic for top sites, app referrers to referrer type report
// 2009-06-01: Upgraded to H.20.3 base code, click depth variables
// 2009-07-07: Added logic to determine visit number per channel
// 2009-10-22: Stopped top sites requests from firing
/************************ ADDITIONAL FEATURES ************************
     Plugins
*/
...
if (typeof(Event)!='undefined' && typeof(Event.onDOMReady)=='function' && typeof($$)=='function' && AC && typeof(AC.Storage)=='object'){
	Event.onDOMReady(function() {
		...
		$$('#globalheader a').each(function(link){
			trackNav(link,'global nav');
		});
		$$('#productheader a').each(function(link){
			trackNav(link,'product nav');
		});
		$$('#globalfooter a').each(function(link){
			trackNav(link,'global footer');
		});
		$$('.productbrowser .pb-slider .pb-slide ul li a').each(function(link){
			trackNav(link,'product browser');
		});
	});
}

//Determine if an AOS campaign, if so tack on store report suite
...
//determining if safari
s._isSafari=false;
if (s.u.toLowerCase().indexOf('webkit')>-1) {
    if(s.u.toLowerCase().indexOf('safari')>-1&&s.u.toLowerCase().indexOf('chrome')<0){
    	s._isSafari=true;
    }
}
...
function safariHandler(link){
	if(s.lt(link.href)){
		link.addEventListener('mousedown', function(evt) {
			if(((evt.which) && (evt.which == 1)) || ((evt.button) && (evt.button == 1))){ //left click only
...
s.linkTrackEvents="None"
/*if (!s.d.URL.match(/apple.com\/welcomescreen/)&&!s.d.URL.match(/apple.com\/(\w{2}|befr|benl|chfr|chde|asia|lae)\/welcomescreen/)){
	s.variableProvider ='DFA#1513429:v46=[["DFA-"+lis+"-"+lip+"-"+lastimp+"-"+lastimptime+"-"+lcs+"-"+lcp+"-"+lastclk+"-"+lastclktime]]';
}*/
...
// Returns the name for tracking this page
// Newer pages use a custom meta tag specifying the exact
// tracking name.
// Legacy pages will use the document.title as a basis for
// the tracking name.
AC.Tracking.pageName = function()
{
	// Return the cached _pageName if it's set
	...
	for (var i = 0, metaTag; (metaTag = metaTags[i]); i++) {
		if ("omni_page" === metaTag.getAttribute("name")) {
			...
			// Only cache a meta element's ptitle, which assuming
			// will never change once a page has loaded
			AC.Tracking._pageName = metaTag.getAttribute("content").toLowerCase();
			return AC.Tracking._pageName;
		}
	}

	// Don't memoize page title on legacy pages, we can't gurantee
	// the title won't change form one tracking attempt to the next
...
AC.Tracking._pageNameForTitle_atHost_andPath = function(title, host, pathName)
{
	...
	// seeing as the Apple "segment" is always delimited by dashes
	// we need to find that segment of the title and rip it out
	//  foo - Apple bar - baz
	// any whitespace -? Space* (Apple Identifier) Space+ Whatever* -? any whitespace
	// be sure to see the omniture page name test, they should all pass
	// for this to be considered correct
	if (/\s-\s/.test(pageName)) {
		pageName = pageName.replace(/\s*-?\s*(apple|アップル|애플컴퓨터코리아|蘋果|蘋果電腦|apple中国|苹果中国)\s+[^-]*-?\s*/, '')
	}

	if (!pathName.match(/^\/(ws|pr|g5|go|ta|wm)\//)) {
		// If the first path segment is two characters but isn't
		// a known exception, it's probably a country
		// so strip them away along with any known country codes 
		// that aren't 2 characters
		pathName = pathName.replace(/^\/(\w{2}|befr|benl|chfr|chde|asia|lae)(?=\/)/, '');
	}

	// top level pages get a " - Index" appended, except support pages
	if ((pathName.match(/\//g).length <= 2) && !pathName.match(/support/) && !host.match(/support/) && !host.match(/selfsolve/) &&
		(!!pathName.match(/index\.html/) || !pathName.match(/\.html/))){
		pageName += " - index";
	}

	// pr pages get a pr prepended, often becasue the title is not
	// hierachical
	if (/\/pr\//.test(pathName)) {
...
function s_doPlugins(s) {
	...
	s.tcall=(typeof(s.linkType)=='undefined')?true:false;
 	...
 	// Convert Smart apostrophes and quotes to regular one
 	if (s.pageName){
    ...
    if (!s.d.URL.match(/(apple.com\/retail\/.+\/map\/|apple.com\/buy\/locator\/|discussions.apple.com|discussionsjapan.apple.com)/g)){
        s.setupDynamicObjectIDs();
    }

	/* Plugin Example: Genesis DFA 0.6 */
	//s.partnerDFACheck("dfa_cookie","dfa","prop44");
	
	// Processor Platform: 2012-05-14
	...
	// Redirect Alias
	s.server=s.getQueryParam('alias');
	if(!s.server){
		s.server="new approach"
	}
	...
	// Campaign Pathing
	s.prop6=(!s.prop6)?('D="'+s.getQueryParam('cp').toLowerCase()+': "+pageName'):s.prop6;
	...
	// Internal Campaigns
	s.eVar7=(!s.eVar7)?s.getQueryParam('aid'):'';
	...
	s.prop14=s.getPreviousValue(s.pageName,"s_pv");
	...
	s.prop38=s.tcall?s.deviceOrientationChanges(true):'';
	
	//Product Ownership (from Chatterbox)
	s.prop32=s.eVar32=s.getQueryParam('psid')
	...
	// Zip code
	s.prop15=s.c_r("PostalCode")?s.c_r("PostalCode"):"no zip"
	
	//Accessed from in store or elsewhere 
	if (!s.prop20) {
		var retailStore = navigator.userAgent.match(/foh:r\d{3}/i);
		s.prop20 = retailStore ? ("store kiosk:" + retailStore.toString().replace(/foh:/i,'')) : "non-store kiosk";
	}
	
	/* Populate eVar11 with file name and fire event15 for enhanced download tracking */
	...
	// Plugin Detections: 2008-09-17: Start
	s.detectRIA('s_ria','prop19','','12','','');
	if(typeof (Media) !='undefined' && s.tcall){
		s.prop18=QTCheck();
	}

 	//mobile and mac os detection
	function osDetect() {
	...
	if(s.tcall){
		...
		if(s.getQueryParam('sr') && s.getQueryParam('vr')){
			var ver=s.getQueryParam('vr');
	...
	}//end membership tracking
	//navigation tracking
	if (AC && typeof(AC.Storage)!="undefined" && s.tcall && !s.prop25) {
		s.prop25=(AC.Storage.getItem('s_nav'))?AC.Storage.getItem('s_nav'):'other nav or none';
	...
	//used to determine the visit number per channel
	if(s.tcall){
		s.prop50=s.getVisitNumPerChannel();
	}
	...
	//populate prop with visitor id
	s.prop49="D=s_vi"
	
	//copy current url in dynamically
	s.prop4=s.prop4?s.prop4:"D=g";
    
	//lowercase all variables, except for one specified in the second parameter
...
s.getVisitNumPerChannel=function(){
	...
	if(!channelMap){ channelMap = '0';}
	
	//remove legacy cookie, write new one	
	s.c_w('s_vnum_' + geoCode,'',63072000);
	s.c_w('s_invisit_'+geoCode,'',63072000);
	s.c_w('s_invisit_n_'+geoCode,'',63072000);
	s.c_w('s_vnum_n_' + geoCode,'',63072000);
	...
	if(channelMap){
		...
		if(!iv) {
			...
			e.setTime(xs);
			s.c_w(c2, (cs?(cs + ',' +channelMap):channelMap), e);
			e.setTime(xl);
			if(lm){
				s.c_w(c, cla.toString(), e);
				return ch + '=' + vn;
			} else {
				(cla.toString())?(cla.push(channelMap + '|' + 1)):(cla=channelMap + '|' + 1);
				s.c_w(c, cla.toString(), e);
				return ch + '=' + 1;
			}
		}
...
+"v'),a=(c.indexOf(',')>-1)?c.split(',',5):[],id=(a.length>0)?(a[0]):"
+"escape(s.getPPVid),cv=(a.length>1)?parseInt(a[1]):(0),p0=(a.length>"
+"2)?parseInt(a[2]):(pv),cy=(a.length>3)?parseInt(a[3]):(0),pt=s._ct,"
+"ph=s._ch,t=new Date;t.setTime(t.getTime()+1800000);s._ct=new Date()"
+".getTime();s._ch=vh;var sa='',td=Math.round((s._ct-pt)/1000),hd=Mat"
+"h.abs(s._ch-ph),lowerBound,upperBound;if(hd&&td){lowerBound=Math.ce"
+"il(st/100)*100;upperBound=Math.ceil(s._ch/100)*100;while(lowerBound"
+"<=upperBound){if(lowerBound!=0){var value=lowerBound+':'+(td>10?'>'"
+":td);if(s.pxViewedArray.length==0){s.pxViewedArray.push(value);}els"
+"e if(s.pxViewedArray.toString().indexOf(lowerBound)==-1){s.pxViewed"
+"Array.push(value);}else{for(i=0;i<s.pxViewedArray.length;i++){var a"
+"v=s.pxViewedArray[i].split(':');if(lowerBound==av[0]){if(av[1]!='>'"
+"){var totalTime=Math.floor((Number(av[1])+Number(td))*100)/100;if(t"
+"otalTime>10){totalTime='>';}s.pxViewedArray[i]=av[0]+':'+totalTime;"
+"}break;}}}}lowerBound=lowerBound+100;s.pxViewedArray.sort(function("
+"a,b){return parseInt(a)-parseInt(b)});}}sa=s.pxViewedArray.toString"
+"().replace(/,/g,'|');cn=(pv>0)?(id+','+((pv>cv)?pv:cv)+','+p0+','+("
+"(vh>cy)?vh:cy)+','+((sa)?sa:'')):'';s.c_w('s_ppv',cn,t);");
s.getPercentPageViewed=new Function("pid",""
+"pid=pid?pid:'-';var s=this,ist=!s.getPPVid?true:false,t=new Date;t."
+"setTime(t.getTime()+1800000);if(typeof(s.linkType)!='undefined'&&s."
+"linkType!='e')return'';var v=s.c_r('s_ppv'),a=(v.indexOf(',')>-1)?v"
+".split(',',5):[];if(a.length<5){for(var i=4;i>0;i--){a[i]=(i<a.leng"
+"th)?(a[i-1]):('');}a[0]='';}a[0]=unescape(a[0]);s.getPPVpid=pid;s.c"
+"_w('s_ppv',escape(pid),t);s.pxViewedArray=[];if(ist){s.getPPVid=(pi"
+"d)?(pid):(s.pageName?s.pageName:document.location.href);s.c_w('s_pp"
+"v',escape(s.getPPVid),t);if(s.wd.addEventListener){s.wd.addEventLis"
+"tener('load',s.handlePPVevents,false);s.wd.addEventListener('scroll"
+"',s.handlePPVevents,false);s.wd.addEventListener('resize',s.handleP"
+"PVevents,false);}else if(s.wd.attachEvent){s.wd.attachEvent('onload"
+"',s.handlePPVevents);s.wd.attachEvent('onscroll',s.handlePPVevents)"
+";s.wd.attachEvent('onresize',s.handlePPVevents);}}return(pid!='-')?"
+"(a):(a[1]);");
/*
...
+"{for(i=0;i<s.d.links.length;i++){l=s.d.links[i];"
+"if(s._isSafari){safariHandler(l);}"
+"c=l[o]?''+l[o]:'';b"
...
s.___se="{}";s.isEntry=new Function(""
...
+"v='';if(!r){return 1;}while(p=l.indexOf(',')){v=p>-1?l.substring(b,"
